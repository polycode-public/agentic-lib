...
| E2 | **Run the kyu benchmark.** **021 done** (`…_021.md`). **022 done** (`…ADVANCED_022.md`): Advanced re-run on the E8/E9 engine — all 3 green (3-kyu 38/38, 2-kyu 108/108, sandbox 2/3). The E8 fixes worked on **first delivery** (demo() auto-wired → distinct screenshots; F8 fires on comments; F9 → autopilot 1-shot; F10 → `sample.html` in-chunk), **but the run exposed a broken re-work loop** (see E10) so the suite didn't get cheaper. Next: E10, then marginalia's Haiku pass. | 021 ✅ · 022 ✅ |
...
| E9 | **FINDING-3 — `on-init` workflow-file push — ✅ DONE + verified.** Root cause: `init.yml` pushed with the Actions `GITHUB_TOKEN` (barred from `.github/workflows`). Fixed: `init.yml` mints the M5 App token (granted `workflows: write`) via OIDC→SSM and pushes with it (`persist-credentials: false` so the checkout token doesn't override it); `on-init.yml` grants `id-token`. Infra provisioned: `/intention/m5/github-app-{id,private-key}` mirrored into **intention-ci** + `m5-ssm-read` inline policy on `intention-fleet-bedrock-role`. **Verified:** `on-init` on sandbox pushed `.github/workflows/*` and opened a PR. | ✅ done |
| E10 | **Fix the re-work loop (from 022 — the headline finding).** Iterative correction is broken: **(F12)** `transform.yml` pushes the `address-review` revision to `agentic-lib/address-review-${WI}` instead of the PR's head branch → it never reaches the PR; **(F11)** the agent burns turns on *denied* debug tools (`node -e`/`cat`) → `error_max_turns`; **(F13)** the only working re-work (`on-intent` re-dispatch) re-delivers the whole chunk → regressions. Also broaden **F9** to cover well-formedness/void-element + metadata self-tests. Until fixed, marginalia's review-driven loop can't iterate. Detail: `reports/BENCHMARK_REPORT_ADVANCED_022.md`. | open (next) |
